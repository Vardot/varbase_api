...
/**
 * Object-oriented hook implementations for Varbase API.
 *
 ...
 * varbase_api.module are kept only as #[LegacyHook] shims that delegate to this
 * service, so the hooks run once and the legacy procedural API stays available.
 */
class VarbaseApiHooks {
   ...
   *   The config installer, used to detect a configuration import.
  ...
  public function entityBundleCreate(string $entity_type_id, string $bundle): void {
    ...
    if (!in_array($entity_type_id, $auto_enabled_entity_types)) {
      ...
      if (isset($existing_entity)) {
        ...
        $config = $this->configFactory->getEditable('jsonapi_extras.jsonapi_resource_config.' . $resource_config_id);
        $config->set('disabled', TRUE)->save();
  ...
  /**
   * Checks if the JSON:API resource config is enabled for this entity.
   *
   * @param \Drupal\Core\Entity\EntityInterface $entity
   *   The entity to check.
   *
   * @return bool
   *   TRUE when the JSON:API resource config is enabled, FALSE otherwise.
   */
  public function isJsonapiResourceConfigEnabled(EntityInterface $entity): bool {
    // Get the JSON:API resource type.
   ...
   *
   * @param \Drupal\Core\Entity\EntityInterface $entity
   *   The entity to build the URL for.
   *
   * @return \Drupal\Core\Url|string
   *   The operation URL, or an empty string when it cannot be built yet.
   */
  public function getViewJsonOperationUrl(EntityInterface $entity): Url|string {
    ...
    if ($uuid && $resource_type) {
      ...
      // @see \Drupal\varbase_api\Hook\VarbaseApiHooks::entityInsert().
      try {
   ...
   *   The operation URL, or an empty string when it cannot be built yet.
   */
  public function getViewApiDocsOperationUrl(EntityInterface $entity): Url|string {
